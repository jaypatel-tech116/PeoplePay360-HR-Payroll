...
import { Layers, Plus, Edit, Trash2, ArrowUp, ArrowDown, Code2, Calculator, Save, PlusCircle, MoreVertical } from 'lucide-react';
  ...
  return (
    <div className="page-content">
      <div className="page-header">
        ...
        <div className="page-title-wrapper">
          <div>
            ...
            <p className="page-subtitle">Define ordered computation sequences (Basic, Allowances, Deductions, Net).</p>
          </div>
          <button
        ...
        <div className="emp-sidebar">
          <div className="card">
            <div className="card-header border-b">
              <h3 className="card-title">Configured Structures</h3>
            </div>
              ...
              <div className="structure-card-list">
                {structures.map((s) => (
                  ...
                  >
                    ...
                    <p className="struct-desc text-muted mt-2">{s.description || 'No description provided'}</p>
                    <div className="struct-meta mt-2">
                      <span className="badge badge-neutral text-xs">{s.rule_count} rules</span>
                      <span className="badge badge-info text-xs">{s.active_employee_count} active contracts</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="emp-content">
          <div className="card h-100">
            <div className="card-header border-b" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h3 className="card-title">{selectedStructure ? selectedStructure.name : 'Select a Structure'}</h3>
                <p className="text-muted text-sm mt-1">Rules execute in strict ascending sequence order.</p>
              </div>
              {selectedStructure && (
                <button className="btn btn-primary" onClick={() => handleOpenRuleModal()}>
                  <PlusCircle size={16} />
                  <span>Add Rule</span>
                </button>
              )}
                ...
                columns={[
                  {
                    ...
                    accessor: 'sequence',
                    render: (r) => (
                      <span className="badge badge-neutral font-mono">{r.sequence}</span>
                    )
                  },
                  {
                    header: 'CODE',
                    accessor: 'code',
                    render: (r) => <strong className="font-mono text-main">{r.code}</strong>
                  },
                  { header: 'RULE NAME', accessor: 'name' },
                  {
                    ...
                    render: (r) => {
                      ...
                      return <span className={`badge ${colors[r.category] || 'badge-neutral'}`}>{r.category.toUpperCase()}</span>;
                    }
                  },
                  {
                    header: 'COMPUTATION',
                    ...
                    render: (r) => {
                      if (r.computation_method === 'fixed') {
                        return <span className="font-medium">Fixed: ₹{parseFloat(r.amount || 0).toLocaleString()}</span>;
                      }
                      if (r.computation_method === 'percentage') {
                        return <span className="font-medium">{r.amount}% of {r.percentage_of_rule_code}</span>;
                      }
                      return <code className="badge badge-neutral formula-preview">{r.formula}</code>;
                    }
                  },
                  {
                    header: 'ACTIONS',
                    ...
                    render: (r) => (
                      <div className="table-actions">
                        <button className="btn-icon" onClick={() => handleOpenRuleModal(r)}>
                          <Edit size={16} />
                        </button>
                        <button className="btn-icon text-danger" onClick={() => handleDeleteRule(r.id)}>
                          <Trash2 size={16} />
                        </button>
                      </div>
                    )
      ...
      <Modal
        ...
        maxWidth="500px"
      >
        <form onSubmit={handleCreateStructure} className="timeoff-modal-form">
          ...
          <div className="modal-footer-actions">
            <button type="button" className="btn btn-outline" onClick={() => setIsStructModalOpen(false)}>
              Cancel
            </button>
      ...
      <Modal
        ...
        maxWidth="600px"
      >
        <form onSubmit={handleSaveRule} className="timeoff-modal-form">
          ...
          <div className="modal-footer-actions">
            ...
            <button type="submit" className="btn btn-primary">
              <Save size={16} />
              <span>{editingRuleId ? 'Update Rule' : 'Add Rule to Sequence'}</span>
            </button>
          </div>
        </form>
      </Modal>
